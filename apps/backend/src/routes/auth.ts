...
async function issueSession(user: { id: string; email: string; name: string; role: { name: string } }, res: Response, rememberMe?: boolean) {
...
authRoutes.post(
  ...
  asyncRoute(async (req, res) => {
    ...
    res.json(await issueSession(user, res, input.rememberMe));
  })
);
